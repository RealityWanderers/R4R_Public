...
public class PlayerLockOnSystemRamenDelivery : PlayerPassive
{
    [Header("Aiming")]
    ...
    [HideInInspector] public PlayerRamenDeliveryPoint ramenDeliveryPoint; 
    ...
    void TryFindTarget()
    {
        ...
        for (int i = 0; i < numTargets; i++)
        {
            ...
            if (collider.TryGetComponent(out PlayerLockOnObjectRamenDelivery lockOnScript))
            {
                    ...
                    if (!Physics.Raycast(cameraPos, directionToTarget, out RaycastHit hit, distanceToTarget, obstructionMask))
                    {
                        if (angleToTarget < closestAngle || (Mathf.Approximately(angleToTarget, closestAngle) && distanceToTarget < closestDistance))
                        {
                            ...
                            if (closestTarget != object_CurrentLockOnTarget)
                            {
                                ...
                                ramenDeliveryPoint = object_CurrentLockOnTarget.GetComponent<PlayerRamenDeliveryPoint>();
