...
namespace RootMotion.FinalIK
{
    ...
    public static class VRIKCalibrator
    {
        ...
        {
            CalibrateScale(ik, false, scaleMlp);
        ...
        {
            CalibrateScale(ik, false, settings.scaleMlp);
        }

        /// <summary>
        /// Calibrates only the avatar scale.
        /// </summary>
        private static void CalibrateScale(VRIK ik, bool useSaveData, float scaleMlp = 1f)
        {
            float scaleToSet = 0; 
            if (useSaveData)
            {
                scaleToSet = PlayerPrefs.GetFloat("CalibrationScale", scaleToSet); 
                ik.references.root.localScale *= scaleToSet;
                //Debug.Log("Used Existing Data:" + scaleToSet);
            }
            else
            {
                ...
                scaleToSet = sizeF * scaleMlp;
                PlayerPrefs.SetFloat("CalibrationScale", scaleToSet);
                ik.references.root.localScale *= scaleToSet;
                //Debug.Log("Saved New Data:" + scaleToSet);
            }
        }
        ...
        //        // Constructor that only stores position and rotation
        //        public Target(Transform t)
        //        {
        //            this.used = t != null;
        //            if (!this.used) return;
        ...
        //        // Reset the target transform with the stored position and rotation
        //        public void SetTo(Transform t)
        //        {
        //            if (!used) return;
        //            t.localPosition = localPosition;
        ...
        /// <returns></returns>
        public static CalibrationData Calibrate(bool saveData, VRIK ik, Transform centerEyeAnchor, Transform leftHandAnchor, Transform rightHandAnchor, Vector3 centerEyePositionOffset, Vector3 centerEyeRotationOffset, Vector3 handPositionOffset, Vector3 handRotationOffset, float scaleMlp = 1f)
        {
            ...
            CalibrateScale(ik, saveData, scaleMlp);
