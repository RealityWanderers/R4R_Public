...
namespace RootMotion.Demos
{
    public class VRIKCalibrationBasic : MonoBehaviour
    {
        ...
        {
            data = VRIKCalibrator.Calibrate(false, ik, centerEyeAnchor, leftHandAnchor, rightHandAnchor, headAnchorPositionOffset, headAnchorRotationOffset, handAnchorPositionOffset, handAnchorRotationOffset, scaleMlp);
        }

        [Button]
        public void CalibrateWithSettings()
        {
            if (data.scale == 0f)
            {
                Debug.LogError("No Calibration Data to calibrate to, please calibrate with 'C' first.");
        ...
        [Button]
        public void CalibrateCharacterScale()
        {
            if (data.scale == 0f)
            {
                Debug.LogError("Avatar needs to be calibrated before RecalibrateScale is called.");
